...
Vertical clipped single-selection list.

**Status:** XML tag: `ListBox`  
**Namespace:** `Dreambit.UI`  
**Source:** `DreambitEngine/UI/Elements/UiListBox.cs`  
**Validated against:** DreambitEngine `main` / `ef6e5b9c600ad6e215c53ea287a0c7858884ce00`

## Inheritance

`UiElement` → `UiContainer` → `UiStackPanelBase` → `UiStackPanel` → `UiItemsControl` → `UiSelector` → `UiListBox`

## When to use

Use for small lists of arbitrary UI item elements that need pointer and keyboard/controller selection.
...
```xml
<ListBox id="save-list"
         width="360" height="240"
         spacing="4"
         selected-index="0"
         background-tint="#151B24">
    <ListBox.Background>
        <SolidColorBrush />
    </ListBox.Background>

    <Button width="100%" height="40" selected-tint="#355C85">
        <Text text="Autosave" font="monogram" />
    </Button>
    <Button width="100%" height="40" selected-tint="#355C85">
        <Text text="Slot 1" font="monogram" />
    </Button>
</ListBox>
```
...
- Orientation is vertical and `ClipToBounds` is enabled by the constructor.

## Production pitfalls

- Clipping does not provide scrolling. Items outside bounds are hidden and cannot be reached through an automatic scroll viewer.
- Use direct `UiControl` items when selected-state tinting is required.
