# `UiVerticalStackPanel`

Concrete top-to-bottom stack panel.

**Status:** XML tag: `VerticalStackPanel`  
...
`UiElement` → `UiContainer` → `UiStackPanelBase` → `UiVerticalStackPanel`

## When to use

Use for menus, forms, settings rows, item columns, and vertically grouped HUD text.
...
```xml
<VerticalStackPanel width="320" height="*"
                    padding="16"
                    spacing="10"
                    cross-alignment="Center"
                    grow-direction="Top">
    <Text width="100%" height="*" text="Settings" font="monogram" />
...
```csharp
var stack = new UiVerticalStackPanel
{
...
- Children are laid out top-to-bottom. Cross alignment controls horizontal placement.
