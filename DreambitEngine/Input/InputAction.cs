        ...
        switch (Type)
        {
            case InputActionType.Button:
            {
                ...
                if (_phase == InputActionPhase.Waiting && pressed)
                {
                    ...
                    _time = 0;
                    Started?.Invoke(new InputActionContext(Name, _time, 1f, default));
                    _phase = InputActionPhase.Performed;
                    Performed?.Invoke(new InputActionContext(Name, _time, 1f, default));
                }
                ...
                {
                    Performed?.Invoke(new InputActionContext(Name, _time, 1f,
                        default)); // fire each frame while held (like Unity's performed)
                }
            ...
            case InputActionType.Value2D:
            {
                ...
                if (v.LengthSquared() < DeadZone * DeadZone) v = Vector2.Zero;
