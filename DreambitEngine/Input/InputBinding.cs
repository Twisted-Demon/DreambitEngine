...
public struct InputBinding
{
    ...
    {
        return new InputBinding(BindingType.Key) { Key = k };
    }

    public static InputBinding MouseType(MouseButton btn)
    {
        return new InputBinding(BindingType.MouseButton) { MouseButton = btn };
    }

    public static InputBinding AxisType(Axis1D axis, float scale = 1f)
    {
        return new InputBinding(BindingType.Axis1D) { Axis1D = axis, Scale = scale == 0 ? 1f : scale };
    }

    public static InputBinding Composite2DKeys(Keys up, Keys down, Keys left, Keys right)
    {
        return new InputBinding(BindingType.Composite2D)
            { Up = up, Down = down, Left = left, Right = right, Axis2D = Axis2D.Keys };
    }

    public static InputBinding Composite2DMouse()
    {
        return new InputBinding(BindingType.Composite2D) { Axis2D = Axis2D.Mouse };
    }

    public static InputBinding ChordType(Keys modifier, Keys primary)
    {
        return new InputBinding(BindingType.Chord) { ModKey = modifier, PrimaryKey = primary };
    }
