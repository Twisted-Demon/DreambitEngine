...
public static class Mathf
{
    ...
    public static float Mod(float x, float y) { // Euclidean, result in [0,y)
        var r = x % y;
        return r < 0f ? r + y : r;
    }
    
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public static float Fract(float x) => x - (int)x; // works for +/-; for large |x| consider bit tricks
    ...
    public static void SinCos(float x, out float s, out float c)
    {
        ...
        x = x - TwoPi * (float)MathF.Round(x * (1f / TwoPi)); // keep this one MathF or replace with your own Round

        // Use odd/even polynomials (Estrin’s scheme)
        float x2 = x * x;
        // Cos approx: c ≈ 1 + x2*(-0.5 + x2*( 0.041666638 - x2*0.0013888378))
        float c2 = FMA(x2, FMA(x2, FMA(x2, -0.0013888378f, 0.041666638f), -0.5f), 1f);
        // Sin approx: s ≈ x + x^3*(-0.16666667 + x2*(0.0083333310 - x2*0.00019840874))
        float s2 = FMA(x2, FMA(x2, -0.00019840874f, 0.0083333310f), -0.16666667f);
        s = FMA(s2, x*x2, x);
        c = c2;
    }
    ...
    {
        a = a - TwoPi * (float)MathF.Floor(a / TwoPi);
