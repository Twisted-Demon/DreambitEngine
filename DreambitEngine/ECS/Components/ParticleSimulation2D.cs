...
public class ParticleSimulation2D : ICanLog<ParticleSimulation2D>
{
    ...
    public ParticleSimulation2D(Transform transform)
    {
        Transform = transform;
    }
    ...
    public void SetParticleFxConfig(ParticleFxConfig config)
    {
        ...
        {
            var maxLife = Math.Max(1e-3f, _config.LifeTime.Max);
        ...
        else // Burst
        {
            ...
            {
                var perCycle = Math.Max(0, b.Count);
        ...
        _ax = new float[capacity];
        _ay = new float[capacity];
        _rot = new float[capacity];
        _spin = new float[capacity];
        _life = new float[capacity];
        _maxLife = new float[capacity];
        _color = new Color[capacity];
        _sx = new float[capacity];
        _sy = new float[capacity];

        _sx0 = new float[capacity];
        _sy0 = new float[capacity];
        _spin0 = new float[capacity];
        _speed0 = new float[capacity];
        _dirx = new float[capacity];
        _diry = new float[capacity];
    ...
    {
        return new ReadOnlyParticles(_indices, _px, _py, _vx, _vy, _sx, _sy, _rot, _life, _maxLife, _color, _spin, _ax,
            _ay, _alive);
    }
    ...
    private void EmitBurst()
    {
        ...
        var cycleStart = _nextBurstWindowStartSec;
        var anyPending = false;

        for (var i = 0; i < _config.Bursts.Count; i++)
        {
            ref var fired = ref _burstCounters[i];
            var b = _config.Bursts[i];
            if (_burstCycle >= b.Cycles && b.Cycles > 0) continue;

            anyPending = true;

            var nextT = _burstNextTimes[i];
    ...
    private void SpawnOne()
    {
        ...
        // Lifetime
        var life = RandRange(_config.LifeTime.Min, _config.LifeTime.Max);

        // Start size
        var size = RandRange2(_config.StartSize.Min, _config.StartSize.Max);

        // Rotation / spin
        var startRotDeg = RandRange(_config.StartRotationDeg.Min, _config.StartRotationDeg.Max);
        var startRot = MathHelper.ToRadians(startRotDeg);
        var spin = RandRange(_config.StartSpin.Min, _config.StartSpin.Max);

        // Start acceleration
        var accel = RandRange2(_config.StartAcceleration.Min, _config.StartAcceleration.Max);

        // Base color
        var color = _config.StartColor;
        ...
        var dir = RandomUnitDirection2D();
        var vel = dir * speed;

        // Apply velocity jitter additively
        var vj = RandRange2(_config.VelocityJitter.Min, _config.VelocityJitter.Max);
        vel += vj;
    ...
    private void SimulateStep()
    {
        ...
        var damping = Math.Max(0f, _config.LinearDamping);
        var dampFactor = damping > 0f ? Mathf.Exp(-damping * dt) : 1f;
        ...
        while (i < _alive)
        {
            ...
            var tNorm = 1f - life / _maxLife[phys]; // 0..1
            tNorm = Math.Clamp(tNorm, 0f, 1f);

            // Over-life curves (evaluate once)
            var sizeScale =
                _config.SizeOverLife.Evaluate(tNorm); // scales base size  :contentReference[oaicite:5]{index=5}
            var speedScale = _config.SpeedOverLife.Evaluate(tNorm); // scales base speed
            var spinScale = _config.SpinOverLife.Evaluate(tNorm); // scales base spin
            var alphaScale = _config.AlphaOverLife.Evaluate(tNorm); // scales alpha

            // Spin update from base * curve
            var spin = _spin0[phys] * spinScale;
            _spin[phys] = spin;
            _rot[phys] += spin * dt;

            // Velocity = (baseDir * baseSpeed * speedScale) + per-particle jitter already embedded
            var vx = _dirx[phys] * _speed0[phys] * speedScale;
            var vy = _diry[phys] * _speed0[phys] * speedScale;
            ...
            var s = 1f - Mathf.Exp(-followRate * dt);
            vcx += (vx - vcx) * s;
            vcy += (vy - vcy) * s;

            // integrate with accel + gravity + damping
            var ax = _ax[phys] + gravity.X;
            var ay = _ay[phys] + gravity.Y;

            vcx = (vcx + ax * dt) * dampFactor;
            vcy = (vcy + ay * dt) * dampFactor;

            var px = _px[phys] + vcx * dt;
            var py = _py[phys] + vcy * dt;
            ...
            var sx = Math.Max(0f, _sx0[phys] * sizeScale);
            var sy = Math.Max(0f, _sy0[phys] * sizeScale);
            _sx[phys] = sx;
            _sy[phys] = sy;
    ...
    private Vector2 SpawnPointBase()
    {
        ...
        switch (_config.SpawnType)
        {
            ...
            case ParticleSpawnType.Circular:
            {
                ...
                var rx = RandRange(rMin.X, rMax.X);
                var ry = RandRange(rMin.Y, rMax.Y);

                // uniform in disk via sqrt for radius
                var ang = RandRange(0f, MathHelper.TwoPi);
                var mag = (float)Math.Sqrt(_rng.NextDouble());
    ...
    private static Vector2 RandomUnitDirection2D()
    {
        ...
        var ang = (float)(RandomShared.NextDoubleStatic() * MathHelper.TwoPi);
    ...
    private bool HasMoreCycles()
    {
        ...
        {
            var b = _config.Bursts[i];
            if (b.Cycles <= 0 || _burstCycle < b.Cycles)
            {
                any = true;
                break;
            }
    ...
    {
        var last = _alive - 1;
        var deadPhys = _indices[logicalIdx];

        var movedPhys = _indices[last];
    ...
    {
        var tx = (float)_rng.NextDouble();
        var ty = (float)_rng.NextDouble();
