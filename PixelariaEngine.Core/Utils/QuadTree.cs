...
public class Quadtree<T>
{
    ...
    private int _level;                 // Current level of this node
    private List<(T Entity, Vector2 Position)> _entities; // Entities in this node
    private Rectangle _bounds;          // Bounding box of this node
    private Quadtree<T>[] _nodes;       // Child nodes (NE, NW, SW, SE)
    ...
    private void Split()
    {
        ...
        _nodes[2] = new Quadtree<T>(_level + 1, new Rectangle(x, y + subHeight, subWidth, subHeight + extraHeight));              // SW
        _nodes[3] = new Quadtree<T>(_level + 1, new Rectangle(x + subWidth, y + subHeight, subWidth + extraWidth, subHeight + extraHeight)); // SE
    }
    ...
    public void Insert(T entity, Vector2 position)
    {
        ...
        if (_entities.Count > MaxObjects && _level < MaxLevels)
        {
            ...
            {
                int index = GetIndex(_entities[i].Position);
                if (index != -1)
                {
    ...
    public bool Remove(T entity, Vector2 position)
    {
        ...
        if (removed && _nodes[0] != null)
        {
            ...
            int totalObjects = _entities.Count;
