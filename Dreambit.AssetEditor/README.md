...
`Dreambit.AssetEditor` is a modern Avalonia desktop authoring tool for Dreambit's serialized assets and blueprints. It is intentionally code-only Avalonia UI because the inspector is generated dynamically from Dreambit's runtime types and serialization metadata.

## What it supports

- Creates any `DreambitAsset` that has a registered `.jsonb` `IAssetLoader`.
- Opens and saves readable semantic source files such as `.sprite`, `.blueprint`, and `.asset`, or Dreambit's JSNB-backed `.jsonb` format.
...
- Lets you choose a project asset root and browse semantic Dreambit asset files and `.jsonb` files in a folder tree.
- Opens project assets by double-clicking them in the explorer and refreshes the tree after saves.
- Supports dragging explorer files onto `DreambitAsset` and blueprint reference fields; semantic Dreambit extensions are retained in project-relative references.
...
- Supports dragging semantic Dreambit asset files, `.json`, `.jsonb`, and `.dll` files onto the editor window.
...
Choose **Set Root** in the Project Explorer and select the asset folder used by your game's content build. For example, dropping `characters/player.blueprint` into a blueprint reference field stores `characters/player.blueprint`.
